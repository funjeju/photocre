export const ko = {
  app: {
    name: 'Framelab',
    tagline: '사진을 감성으로, 시리즈로.',
  },
  nav: {
    studio: 'Studio',
    cover: 'Cover',
    magazine: 'Magazine',
    dream: 'Dream',
    history: 'History',
  },
  auth: {
    loginTitle: '당신만의 감성을\n한 번의 클릭으로,\n계속 이어가세요.',
    loginDescription:
      'Framelab은 프레임·배경·스타일·텍스트를 조합해 감성 이미지를 만들고,\n템플릿으로 저장해 다음 사진에도 동일한 느낌을 빠르게 적용할 수 있는 AI 이미지 스튜디오입니다.',
    loginButton: 'Google로 시작하기',
    loginError: '로그인에 실패했습니다. 다시 시도해주세요.',
    loggingIn: '로그인 중...',
  },
  credits: {
    badge: '{{count}}크레딧',
    insufficient: '크레딧이 부족합니다.',
    dailyReset: '크레딧은 매일 00:00 KST에 초기화됩니다.',
  },
  studio: {
    title: 'Studio',
    uploadPrompt: '사진을 드래그하거나 클릭해서 업로드',
    uploadHint: 'JPG, PNG, WebP, HEIC · 최대 10MB',
    uploadSizeError: '파일 크기가 10MB를 초과합니다.',
    uploadFormatError: '지원하지 않는 파일 형식입니다.',
    crop: {
      title: '크롭',
      confirm: '확인',
      cancel: '취소',
    },
    frame: {
      title: '프레임',
      seeAll: '전체보기',
    },
    background: {
      title: '배경',
      seeAll: '전체보기',
      customUpload: '업로드',
    },
    style: {
      title: '스타일',
      seeAll: '전체보기',
    },
    ai: {
      title: 'AI 스타일 변환',
      credit: '크레딧 1',
      prompt: {
        label: '추가 요구사항 (선택)',
        placeholder: '예: 배경을 저녁노을로, 표정을 웃게, 조명을 따뜻하게',
      },
    },
    decorate: {
      title: '꾸미기',
      lockedHint: 'AI 변환 후 활성화',
    },
    text: {
      title: '텍스트',
      placeholder: '텍스트를 입력하세요 (선택사항)',
      fontFamily: '폰트',
      fontSize: '크기',
      color: '색상',
      alignment: '정렬',
    },
    generate: {
      button: 'AI 변환하기',
      generating: 'AI 변환 중...',
      noAI: '원본 그대로 사용',
      creditsRequired: '크레딧 1개가 사용됩니다.',
    },
    result: {
      download: '다운로드',
      regenerate: '다시 생성',
      saveToProfile: '마이페이지 저장',
      newImage: '새 이미지',
      share: '공유',
      synthIdNotice: 'AI로 생성된 이미지입니다.',
    },
    mockup: {
      title: '굿즈 미리보기',
      note: '이미지로 만들 수 있는 굿즈 시뮬레이션 (AI 비용 없음)',
      tshirt:     '티셔츠',
      mug:        '머그컵',
      cushion:    '쿠션',
      totebag:    '에코백',
      griptok:    '그립톡',
      minicanvas: '미니캔버스',
    },
  },
  dream: {
    title: 'Dream',
    subtitle: '내 사진으로 미래 직업을 AI가 그려드립니다',
    photoLabel: '내 사진 업로드',
    photoHint: '얼굴이 잘 보이는 사진 권장 · JPG·PNG·WebP',
    careerLabel: '꿈꾸는 직업',
    careerPlaceholder: '예: 우주 비행사, 건축가, 유튜버, 게임 개발자...',
    ageLabel: '상상할 나이',
    generateButton: '드림 생성하기',
    generating: 'AI가 미래를 그리는 중...',
    creditNote: '크레딧 1개 사용',
    resultTitle: '미래의 나',
    strengths: '나의 강점',
    skills: '필요 스킬',
    careerPath: '커리어 경로',
    encouragement: '응원 메시지',
    downloadImage: '이미지 저장',
    pdfButton: 'PDF 저장',
    resetButton: '새로운 꿈 만들기',
    noPhoto: '사진을 먼저 업로드해주세요.',
    noCareer: '꿈꾸는 직업을 입력해주세요.',
    myDreams: '나의 Dream 기록',
    dreamsEmpty: 'Dream 기록이 없습니다.',
    dreamsEmptyHint: 'Dream 탭에서 나의 미래 직업을 AI로 미리 만나보세요.',
  },
  apply: {
    title: 'Apply',
    selectTemplate: '템플릿을 선택해주세요',
    uploadPhotos: '사진을 업로드하세요 (최대 20장)',
    generateAll: '전체 생성',
    downloadAll: '전체 ZIP 다운로드',
    status: {
      waiting: '대기 중',
      cropping: '크롭 중',
      generating: '생성 중',
      done: '완료',
      failed: '실패',
    },
  },
  templates: {
    title: 'Templates',
    empty: '저장된 템플릿이 없습니다.',
    emptyHint: 'Studio에서 이미지를 생성하고 템플릿으로 저장해보세요.',
    rename: '이름 변경',
    delete: '삭제',
    setDefault: '기본 설정',
    usedCount: '{{count}}회 사용',
    saveDialog: {
      title: '템플릿으로 저장',
      namePlaceholder: '템플릿 이름',
      confirm: '저장',
      cancel: '취소',
    },
  },
  magazine: {
    title: 'Magazine',
    subtitle: '사진을 올리면 매거진 레이아웃이 자동으로 생성됩니다.',
    uploadLabel: '사진을 드래그하거나 클릭해서 업로드',
    uploadHint: 'JPG, PNG, WebP · 최대 4장',
    selectedCount: '{{n}}장 선택됨',
    generateButton: '매거진 생성하기',
    resultsTitle: '매거진 결과',
    resultsHint: '마음에 드는 결과를 선택하세요',
    generating: '생성 중...',
    download: 'PNG 다운로드',
    downloading: '고화질 저장 중...',
    regenerate: '새롭게 구성하기',
    downloadDone: '다운로드 완료!',
    downloadError: '다운로드 실패. 다시 시도해주세요.',
    previewError: '미리보기 로드 실패',
    noImage: '사진을 최소 1장 업로드해주세요.',
    renderError: '일부 결과 생성에 실패했습니다.',
  },
  history: {
    title: 'History',
    empty: '생성 히스토리가 없습니다.',
    emptyHint: 'Studio에서 이미지를 생성하면 여기에 표시됩니다.',
  },
  shop: {
    noImage: '먼저 Studio에서 이미지를 생성해주세요.',
    goStudio: 'Studio로 이동',
    orderButton: '주문하기',
    addToCart: '장바구니 담기',
    buyNow: '바로 구매',
    options: '옵션 선택',
    quantity: '수량',
    price: '가격',
    shippingFee: '배송비',
    freeShipping: '무료배송',
    total: '합계',
    deliveryDays: '{{n}}일 이내 제작·발송',
    customImage: '내 커스텀 이미지',
  },
  cart: {
    title: '장바구니',
    empty: '장바구니가 비어있습니다.',
    emptyHint: 'Studio에서 이미지를 생성하고 굿즈를 주문해보세요.',
    remove: '삭제',
    checkout: '주문하기',
    continueShopping: '쇼핑 계속하기',
    itemCount: '{{n}}개 상품',
  },
  checkout: {
    title: '배송지 입력',
    name: '수령인',
    phone: '연락처',
    zipCode: '우편번호',
    searchAddress: '주소 검색',
    address: '기본 주소',
    addressDetail: '상세 주소',
    memo: '배송 메모 (선택)',
    memoPlaceholder: '예: 문 앞에 놓아주세요',
    orderSummary: '주문 내역',
    paymentMethod: '결제 방법',
    bankTransfer: '무통장입금',
    bankInfo: '주문 후 입금 계좌 안내',
    placeOrder: '주문 완료',
    placing: '주문 처리 중...',
    required: '필수 항목입니다.',
    phonePlaceholder: '010-0000-0000',
  },
  order: {
    title: '주문 내역',
    detail: '주문 상세',
    empty: '주문 내역이 없습니다.',
    emptyHint: 'Studio에서 이미지를 만들고 첫 주문을 해보세요.',
    orderId: '주문번호',
    orderedAt: '주문일',
    status: {
      received:   '주문 접수',
      processing: '제작 중',
      shipped:    '배송 중',
      delivered:  '배송 완료',
      cancelled:  '취소됨',
    },
    payment: {
      pending:   '입금 대기',
      paid:      '결제 완료',
      cancelled: '결제 취소',
      refunded:  '환불 완료',
    },
    shipping: {
      carrier:  '택배사',
      tracking: '송장번호',
      none:     '배송 전',
    },
    placed: '주문이 완료되었습니다.',
    placedHint: '입금 확인 후 제작이 시작됩니다.',
    bankAccount: '입금 계좌',
    goOrders: '주문 내역 보기',
  },
  admin: {
    title: '어드민',
    orders: '주문 관리',
    products: '상품 관리',
    totalOrders: '전체 주문',
    pendingPayment: '입금 대기',
    processing: '제작 중',
    shipped: '배송 중',
    markPaid: '결제 확인',
    markProcessing: '제작 시작',
    markShipped: '배송 처리',
    markDelivered: '배송 완료',
    markCancelled: '취소 처리',
    enterTracking: '송장 입력',
    carrier: '택배사',
    trackingNumber: '송장번호',
    save: '저장',
    mockupEditor: '굿즈 배치 설정',
    adminMemo: '관리자 메모',
    productName: '상품명',
    basePrice: '기본 가격',
    isActive: '판매 여부',
    active: '판매 중',
    inactive: '판매 중지',
  },
  profile: {
    title: '마이페이지',
    myImages: '내 생성 이미지',
    empty: '생성한 이미지가 없습니다.',
    emptyHint: 'Studio에서 이미지를 생성하면 여기에 저장됩니다.',
    orderWith: '이 이미지로 주문',
    generatedAt: '생성일',
    plan: {
      free:     'Free',
      personal: 'Personal',
      pro:      'Pro',
    },
    folder: {
      all: '전체',
      newFolder: '새 폴더',
      createTitle: '폴더 만들기',
      createPlaceholder: '폴더 이름',
      renamePlaceholder: '새 폴더 이름',
      deleteConfirm: '폴더를 삭제하면 안에 있던 이미지들은 전체로 이동됩니다.',
      moveTo: '폴더로 이동',
      rename: '이름 변경',
      delete: '폴더 삭제',
      noImages: '이 폴더에 이미지가 없습니다.',
    },
    gen: {
      editLabel: '이름 변경',
      labelPlaceholder: '이미지 이름 (선택)',
      moveToFolder: '폴더로 이동',
      deleteImage: '이미지 삭제',
      deleteConfirm: '이 이미지를 삭제할까요? 복구할 수 없습니다.',
      download: '이미지 저장',
    },
  },
  errors: {
    unknown: '오류가 발생했습니다. 다시 시도해주세요.',
    network: '네트워크 오류가 발생했습니다.',
    unauthorized: '로그인이 필요합니다.',
  },
} as const;

export type KoStrings = typeof ko;
